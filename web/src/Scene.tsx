...
function nacaPoints(state: GeometryState, samples = 80): THREE.Vector2[] {
...
function planarShape(state: GeometryState): THREE.BufferGeometry {
  ...
  switch (state.id) {
    case 'circle': shape.absarc(0, 0, p('radius'), 0, Math.PI * 2, false); return new THREE.ExtrudeGeometry(shape, { depth: 0.04, bevelEnabled: false });
    case 'rectangle': shape.moveTo(-p('length') / 2, -p('width') / 2); shape.lineTo(p('length') / 2, -p('width') / 2); shape.lineTo(p('length') / 2, p('width') / 2); shape.lineTo(-p('length') / 2, p('width') / 2); shape.closePath(); return new THREE.ExtrudeGeometry(shape, { depth: 0.04, bevelEnabled: false });
    case 'triangle': shape.moveTo(-p('base') / 2, 0); shape.lineTo(p('base') / 2, 0); shape.lineTo(0, p('height')); shape.closePath(); return new THREE.ExtrudeGeometry(shape, { depth: 0.04, bevelEnabled: false });
    case 'ellipse': shape.absellipse(0, 0, p('semiMajor'), p('semiMinor'), 0, Math.PI * 2, false, 0); return new THREE.ExtrudeGeometry(shape, { depth: 0.04, bevelEnabled: false });
...
function createShape(state: GeometryState): THREE.BufferGeometry {
  ...
  switch (state.id) {
    case 'naca0012': case 'naca2412': case 'naca4412': case 'flatPlate': {
      const shape = new THREE.Shape();
      if (state.id === 'flatPlate') { shape.moveTo(0, 0); shape.lineTo(p('chord'), 0); shape.lineTo(p('chord'), 0.02 * p('chord')); shape.lineTo(0, 0.02 * p('chord')); shape.closePath(); }
      else { const pts = nacaPoints(state); shape.moveTo(pts[0].x * p('chord'), pts[0].y * p('chord')); for (const point of pts.slice(1)) shape.lineTo(point.x * p('chord'), point.y * p('chord')); shape.closePath(); }
